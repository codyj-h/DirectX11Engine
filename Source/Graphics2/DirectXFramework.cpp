...
XMMATRIX DirectXFramework::GetViewTransformation()
{
	...
	return  DirectXFramework::GetDXFramework()->GetCamera()->GetViewMatrix();
}
...
void DirectXFramework::OnResize(WPARAM wParam)
{
	...
	XMStoreFloat4x4(&_projectionTransformation, XMMatrixPerspectiveFovLH(XM_PIDIV4,
		(float)GetWindowWidth() / GetWindowHeight(), 1.0f, 10000.0f));
